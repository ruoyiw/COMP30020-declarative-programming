...
data TestResult
    ...
    | Fail Double String String
    -- | Test threw an exception.
    | Exception Double SomeException
...
data TestCase
    ...
    | Ratio TestCase
    -- | The specified function is applied to the marks computed for 
    -- the enclosed test(s).
    | Computed (Double -> Double) TestCase
...
expr `expect` expected
    = Test 0 (if expr == expected then Succeed "" ""
              else Fail 0 "" ("Expected " ++ (show expected) ++ 
                              " but got " ++ (show expr)))
...
quality expr test assessment
    = Test 0 (if not $ test expr then Fail 0 "invalid output" ""
...
-- the quality of the test result.  If the test does not complete, the
-- final argument is the appropriate indication of quality.
cumulative :: Show a => a -> (a -> Bool) -> (a -> Double) -> Double -> TestCase
cumulative expr test assessment badValue
    = Test badValue (if not $ test expr then Fail badValue "invalid output" ""
...
runTest fhandle l n limit verbose (ResultFile file tcase) = do
    ...
    return (n1,t,c)
runTest fhandle l n limit verbose (Test badValue code) =
    actualTest fhandle l n limit verbose badValue code 
...
actualTest :: Handle -> String -> Int ->  Maybe Double -> Bool 
           -> Double -> TestResult -> IO (Int,Double,Double)
actualTest fhandle l n limit verbose badValue code =
    do printLabel fhandle l n
       result0 <- timeout (timeoutTime limit) 
                  (limitedTest fhandle n verbose badValue code)
       case result0 of
         Nothing ->             -- timed out:  haven't printed message yet
             do printTestResult fhandle verbose $ Timeout badValue
                return (n+1,testResult $ Timeout badValue, 1)
...
limitedTest fhandle n verbose badValue code =
    -- force evaluation; catch any exceptions
              ...
              (\e -> handleTestResult fhandle n verbose
                     (Exception badValue (e::SomeException)))
...
resultMessage verbose (Fail _ norm verb) 
    = "FAILED***    "  ++ (parenthetical $ pick verbose norm norm++verb)
resultMessage verbose (Exception _ e) 
...
resultMessage verbose (Cumulative score norm verb) 
    = "PASSED " ++ (show score) ++ " " ++
      (parenthetical $ pick verbose norm norm++verb)
resultMessage _ (Timeout _) = "TIMEOUT**"

testResult (Succeed _ _)          = 1.0
testResult (Fail badValue _ _)    = badValue
testResult (Exception badValue _) = badValue
testResult (Quality val _ _)      = val
testResult (Cumulative val _ _)   = val
testResult (Timeout badValue)     = badValue
