...
-- Custom testing:
tester :: [String] -> Int -> IO ()
tester target attempts = do
  if length target == 3 && validChord target then do
    let (guess,other) = initialGuess
    loop target guess other attempts 1
    else do
    putStrLn $ "Whoops!"
    exitFailure
...
loop :: [String] -> [String] -> Proj1.GameState -> Int -> Int -> IO ()
loop target guess (GameState targs) attempts guesses = do
  if (attempts /= 0) then do
    ...
    if (validChord guess) then do
      ...
      if answer == (3,0,0) then do
        ...
        let (guess',other') = nextGuess (guess, (GameState targs)) answer
        loop target guess' other' (attempts - 1) (guesses + 1)
      else do
      putStrLn "Invalid guess"
  else do
    putStrLn "Ran out of guesses"
